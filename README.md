...
2. **SSH key** — generate with `ssh-keygen -C cai`, move to `~/.ssh/`, and add the public key under **Remote Editing** in Cloudera AI.
...
If step 6 was skipped, login may succeed but SSH/Cursor connection will fail.

See the [full setup guide](https://superellipse.github.io/zeta-hol/vibe-coding/cursor-remote-ssh/) for screenshots and troubleshooting.

## Quick start

```bash
cp connect-cai.env.example connect-cai.env
# Edit connect-cai.env — set CAI_DOMAIN, CAI_USERNAME, API_KEY, PROJECT_NAME
